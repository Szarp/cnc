...
var //facebook = require('./facebookComunication.js'),
    ...
    grbl = require('./cncJsonParser.js');
                   
                   var buff = new Array();
var command= new Array();
var analizePlace = new grbl.analizePlace();
var serialTransfer =new grbl.serialTransfer(analizePlace);
var grblMove = new grbl.gcode(analizePlace,serialTransfer);
//serialTransfer.begin();
...
function postCall(userId,body,callback){
    ...
    //var body=req.body;
        ...
        if(body.mode=='getSettings'){
        ...
        else if(body.mode=='cnc'){
                //console.log('response Changes')
            if(body['type']=='move'){
                var pos = grblMove.fromArr(body['value']);
                grblMove.moveAbsolute(pos);
                var obj={
                    value:grblMove.readPos(),
                    type:"pos"
                }
                res=obj;
            }
            ...
            else if(body['type']=='setZero'){
                //console.log("ssetZero");
                //var pos = grblMove.fromArr(body['value']);
                grblMove.clearPos();
                var obj={
                    value:grblMove.readPos(),
                ...
                var pos = grblMove.fromArr(body['value']);
                console.log(pos);
                var last = grblMove.readPos();
                var status = true;
                var y,i =0;
                for(k in pos){
                    y=pos[k];
                    console.log("pos",Math.abs(y-last[k]));
                    if(Math.abs(y+last[k])>howManySteps){
                        status = false;
                        break;
